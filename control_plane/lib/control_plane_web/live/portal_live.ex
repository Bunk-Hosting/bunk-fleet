defmodule ControlPlaneWeb.PortalLive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok, socket |> assign(regions: Fleet.list_regions(), sizes: @sizes) |> load_vpses()}
  end
    ...
    cond do
      ...
      true ->
        case Provisioning.create_vps_for_owner(socket.assigns.current_user, attrs) do
          {:ok, _} ->
            {:noreply, socket |> put_flash(:info, "VPS wordt aangemaakt…") |> load_vpses()}

          {:error, :quota_exceeded} ->
            {:noreply, put_flash(socket, :error, "Je hebt je maximum aantal VPS-servers bereikt.")}

          {:error, :no_capacity} ->
            {:noreply, put_flash(socket, :error, "Geen capaciteit beschikbaar in die regio.")}

          {:error, :pool_exhausted} ->
            {:noreply, put_flash(socket, :error, "Geen IP-adressen meer beschikbaar.")}

          {:error, _} ->
            {:noreply, put_flash(socket, :error, "Kon de VPS niet aanmaken.")}
        end
    end
  end
  ...
  def render(assigns) do
    ...
    <div class="wrap">
      ...
      <form phx-submit="create" class="create-form">
        ...
        <select name="size">
          <option :for={{k, s} <- @sizes} value={k}>{s.label}</option>
        </select>
