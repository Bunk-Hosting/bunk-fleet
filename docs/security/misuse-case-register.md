...
| ~~**O-7**~~ | — | ~~No email verification + automatic €10 signup bonus → throwaway-account credit/compute farming.~~ **FIXED 2026-09-09** — the bonus is granted by `Accounts.confirm_user/1` on a verified email-link click, not at registration (`register_user/1` sends a confirmation email instead; `Accounts.deliver_user_confirmation_instructions/1` invalidates any earlier unconsumed link on resend). A throwaway address that never receives/opens the mail never gets credited. VPS provisioning itself is deliberately NOT gated on confirmation — that would block a legitimate impatient user from ever trying the product, which O-7 never required. | accounts.ex, notifier.ex, mailer.ex |
...
**Highest-leverage now that O-24, O-10 and O-7 are closed:** O-39 (CF-Connecting-IP not allowlisted at the edge) as defence-in-depth is the top remaining item, followed by the low-severity quick wins (per-account login lockout, token in localStorage, CSV formula injection, admin table pagination). O-9 has since been fixed: `create_vps_for_owner` charges the server-authoritative package price.
