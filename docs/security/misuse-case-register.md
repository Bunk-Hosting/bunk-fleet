...
**Highest-leverage now that O-24 is closed:** **O-10** (metered usage is never actually debited, and nothing suspends a VPS at zero balance — the wallet leaks) and **O-7** (unverified signup + automatic €10 bonus is farmable). O-9 has since been fixed: `create_vps_for_owner` charges the server-authoritative package price.
